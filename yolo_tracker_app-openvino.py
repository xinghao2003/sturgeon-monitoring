...
DEFAULT_DEVICE = "intel:gpu"
DEFAULT_MODEL_PATH = "yolo11_openvino_model/"
GUI_REFRESH_MS = 30
...
class YOLOTrackerApp:
    ...
    def _build_layout(self):
        ...
        # Device is now hardcoded, display as label instead of dropdown
        Label(dt_row, text=DEFAULT_DEVICE, font=("Segoe UI", 10, "bold"), fg="#0066cc").grid(row=0, column=1, padx=4, pady=2, sticky="w")
        ...
        self.load_model_btn = Button(btns_row, text="Load Model (.pt/.onnx)", command=self.load_model)
        self.load_model_btn.grid(row=0, column=0, padx=4, pady=2, sticky="w")
    ...
    def _auto_load_model(self):
        """Auto-load model from DEFAULT_MODEL_PATH if it exists."""
        model_dir = Path(DEFAULT_MODEL_PATH)
        if model_dir.exists() and model_dir.is_dir():
            # Look for .onnx or .pt files
            onnx_files = list(model_dir.glob("*.onnx"))
            pt_files = list(model_dir.glob("*.pt"))
            model_file = onnx_files[0] if onnx_files else (pt_files[0] if pt_files else None)
            
            if model_file:
                self.status_text.set(f"Auto-loading model: {model_file.name}…")
                self.master.update_idletasks()
                try:
                    self.model = YOLO(str(model_file), task="segment")
                    self.model_path = str(model_file)
                    self.status_text.set(f"Auto-loaded: {model_file.name}")
                    ...
                    self.model = None
                    self.model_path = None
                    self.status_text.set(f"Failed to auto-load model: {e}")
                    self.load_model_btn.config(state="normal")
    ...
    def start_tracking(self):
        ...
        # Device is hardcoded to intel:gpu
        applied_device = DEFAULT_DEVICE
        try:
